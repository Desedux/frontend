...
export async function listComments(cardId: number, opts?: { parentId?: number; page?: number; limit?: number }) {
...
export async function setReaction(cardId: number, commentId: number, action: ReactionDto['action']) {
  return http<ReactionResponseDto>(`/commentary/${cardId}/${commentId}/reaction`, {
    method: 'PATCH',
    body: { action },
  });
}
