...
export default function CategoryPage() {
  ...
  const handleVote = (postId: number, voteType: "up" | "down") => {
    setPosts(prevPosts =>
      prevPosts.map(post =>
        post.id === postId ? { ...post, votes: voteType === "up" ? post.votes + 1 : post.votes - 1 } : post,
      ),
    )
    voteCard(String(postId), voteType === "up").catch(() => {
      setPosts(prevPosts =>
        prevPosts.map(post =>
          post.id === postId ? { ...post, votes: voteType === "up" ? post.votes - 1 : post.votes + 1 } : post,
        ),
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="space-y-4">
          {sortedPosts.length > 0 ? (
            sortedPosts.map(post => <PostCard key={post.id} post={post} onVote={handleVote} />)
          ) : (
